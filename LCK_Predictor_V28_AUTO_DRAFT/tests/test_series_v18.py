...
def test_series_v18(server):
    ...
    # A versão antiga exigia series_order != map_order. Que as duas ordens
    # divirjam depende do confronto: quando o valor de série e o de mapa
    # concordam sobre todos os candidatos, coincidir é o resultado correto — e
    # o teste acusava regressão onde não havia. O que precisa valer sempre é
    # que a camada de série avalie os mesmos candidatos e produza um valor
    # próprio, não que reordene.
    assert sorted(series_order)==sorted(map_order)
    assert any(abs(x["series_probability_root"]-x["robust_probability_root"])>1e-9
               for x in r["results"]), "a camada de série não alterou nenhuma avaliação"
