...
spec=importlib.util.spec_from_file_location("v16",ROOT/"server.py");m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
...
print("V16 Flex Tree test: OK")
print("runtime_sec",round(elapsed,2),"states",r["model_states_evaluated"])
print("flex_candidates",[(x["root_action"]["champion"],x["root_action"]["role_uncertainty"]) for x in r["results"]])
